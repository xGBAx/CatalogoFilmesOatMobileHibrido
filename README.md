...
* **Teste Manual Exploratório:** O app foi testado simulando falha de rede para forçar os erros 400/500 visuais na tela, foi testado se o app conseguia carregar os filmes da API, foi testado favoritar um filme e foi acessado o historico para conferir se estava sendo salvo.
...
2. No terminal, execute `npm install` para baixar todas as dependências.
3. Inicie o servidor do Expo com `npx expo start`.
4. Leia o QR Code com o aplicativo Expo Go no seu celular, ou pressione `a` para abrir no emulador Android. 
5. Para executar os testes automatizados, rode `npm test`.
